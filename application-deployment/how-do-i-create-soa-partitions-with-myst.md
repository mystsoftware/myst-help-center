...
Alternatively, you could use the MyST Studio  `Control` > `Custom` option under a given Platform Instance to create the partition directly before deployment using `create-partition` action and setting the name as a property (e.g. `-Dsca.composite.partition=Stock`).